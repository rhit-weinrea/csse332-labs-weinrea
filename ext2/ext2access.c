...
os_bool_t fetch_inode(os_uint32_t inode_number, int fd,
  ...
  // has number 1, **not** number 0.  (Thanks a lot, ext2!)
   ...
   os_uint32_t inode_offset = inode_number - 1;
 os_uint32_t blockgroup_num = inode_offset / metadata->inodes_per_group;
  // Step 2.  Figure out the offset of the inode within
  // the group (i.e., is this the 0th inode in the group?
  // the 10th?  the 100th?)
  os_uint32_t offset_within_blockgroup = inode_offset % metadata->inodes_per_group; // you must fix this.
